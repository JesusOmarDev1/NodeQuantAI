"""
prueba.py - Modelado predictivo con Stacking Ensemble
==============================================================
...
from regression.scripts.optimizar_prueba import detectar_overfitting, seleccionar_features_rfecv
...
def _obtener_config_modelo(target_slug):
    if target_slug == "volumen":
        estimators = [
            ...
            # Agregamos la Red Neuronal (MLP)
            #("mlp", MLPRegressor(max_iter=1500, early_stopping=True, random_state=42))
        ]
        nombre = "(LGBM + GB + RF)"
    ...
    pipe = Pipeline([
        # MinMaxScaler en lugar de StandardScaler
        ("scaler", MinMaxScaler()),
...
def seleccionar_features(X, y_target, usar_rfecv=True):
    """
    Selecciona estrictamente las 3 features más importantes para el experimento.
    """
    ...
    # --- PASO 3: SELECCIÓN ESTRICTA (TOP 3) ---
    print("        [Experimento Académico] Extrayendo estrictamente las 3 variables más importantes...")
    from sklearn.ensemble import RandomForestRegressor

    # Usamos un Random Forest rápido para evaluar la importancia real de lo que quedó
    evaluador = RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=None)
    evaluador.fit(X_filt, y_transformado)

    importancias = pd.Series(evaluador.feature_importances_, index=X_filt.columns)

    # Recortamos drásticamente al Top 3
    top_3_cols = importancias.nlargest(3).index.tolist()

    print(f"        -> Top 3 seleccionadas: {top_3_cols}")
...
def optimizar_con_optuna_regresion(X_train, y_train, pipe_tmpl, n_trials=30):
    ...
    def objective(trial):
        ...
        # RANGOS ANTI-MEMORIZACIÓN ÁRBOLES
        ...
        # [EXPERIMENTO ACADÉMICO]: Rangos para la Red Neuronal
        #mlp_hidden = trial.suggest_categorical("mlp_hidden_layers", [(50,), (100,), (50, 25)])
        #mlp_alpha = trial.suggest_float("mlp_alpha", 0.0001, 0.1, log=True)
        ...
        try:
            pipe.set_params(
                ...
                # Inyectamos los hiperparámetros del MLP
                #model__regressor__mlp__hidden_layer_sizes=mlp_hidden,
                #model__regressor__mlp__alpha=mlp_alpha
            )
    ...
    # Asignación final de los mejores parámetros
    mejor_pipe.set_params(
        ...
        # Guardamos los ganadores del MLP
        #model__regressor__mlp__hidden_layer_sizes=estudio.best_params["mlp_hidden_layers"],
        #model__regressor__mlp__alpha=estudio.best_params["mlp_alpha"]
    )
